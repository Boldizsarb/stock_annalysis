...
def asml_prediction(): ### very first one
    ...
    st.write("The data in this script is trained on a neural network, specifically a type of recurrent neural network (RNN) known as Long Short-Term Memory (LSTM)")
    st.write("LSTM is highly advantageous in stock price prediction as it retains short-term and long-term patterns, crucial for understanding erratic and non-linear fluctuations in stock prices.")
    ...
    st.write("This RMSE value is a crucial metric for evaluating the performance of the LSTM model. It gives a quantitative measure of how well the model is performing.")
...
def asml_prediction_forecast(days_to_forecast): ## the parameter is the what is the time interval, how long into the future!
    ...
    st.write("The LSTM forecasting algorithm is an advanced technique that utilizes the LSTM's capacity to comprehend and anticipate sequential data.")


def asml_regression():# regression uses  'Close' price based on 'Open', 'High', 'Low', 'Volume'
    ...
    plt.show()
    """
    st.write("By including the 'Open,' 'High,' 'Low,' and 'Volume' data in addition to the 'Close' price, the model is equipped with a broader range of information regarding the stock's performance.")
    st.write("The model has the ability to acquire knowledge from the associations among these characteristics and the target variable (in this instance, the 'Close' price) in order to enhance the precision of its predictions.")
...
def asm_regression_forecast(days_to_forecast = 30):
...
def forest_model():   ##### FOREST FOREST ### FOREST FOREST ### FOREST FOREST ### FOREST FOREST ### FOREST FOREST ### FOREST FOREST
    ...
    plt.show()
    """
    st.write('True vs. Predicted Stock Prices:')
...
def facebook2_model():
    ...
    st.write("The Prophet model by Facebook recognizes uncertainties in time series data due to factors like seasonal patterns, holidays, and external events. To handle these uncertainties, Prophet includes uncertainty intervals")
    st.write("A wider interval suggests higher uncertainty, while a narrower interval indicates greater confidence in the prediction")

    # Display
    st.write('Stock Price Prediction:')
    st.line_chart(forecast.set_index('ds')[['yhat', 'yhat_lower', 'yhat_upper']])
    st.write("**Interpretation:**")
    st.write("**yhat:**  This is the central forecast or the predicted value in this case the stock price, for a specific date in the future.")
    st.write("**yhat_lower:** The minimum expected value for a future variable is denoted by this. It provides a range within which the actual value is expected to fall with a certain level of certainty. It is calculated as a percentile of the projected values.")
    st.write("**yhat_upper:** It represents the upper limit or expected maximum value for the desired variable on a future date. Similar to yhat_lower, it provides a range within which the actual value is expected to fall with a certain level of confidence")
...
def arima(forecast_months = 3):
    ...
    st.write("he ARIMA model has three main parameters includes p lag observations, d differences, and a moving average window of size q.")
    st.write("These parameters are being determined by testing the models first, and choosing the most suitable for the data.")
    fig = go.Figure()
    # Historical data
    fig.add_trace(
    ...
    st.write(f"Mean Squared Error (MSE): {mse_value}")
    st.write(f"Mean Absolute Error (MAE): {mae_value}")
    st.write("RMSE and MSE are measures of the average error between the predicted values and the actual values. They are widely used to evaluate the accuracy of forecasting models. Lower values of RMSE and MSE indicate better model performance")
    st.write("The Values are being measured on the trained data rather than the forecasted one to not influence the measurements.")
